...
registerMinigame('simulation', function(ct, data) {
  ...
  if (scenarios.length > 0) {
    ...
    function renderScenario() {
      ...
      if (phase === 'results') {
        ...
        h += '<div class="mg-complete">' + (_ui.complete || '¡Completado!') + '</div>';
        h += '<div style="font-size:36px;font-weight:700;color:' + acc + ';margin:12px 0">' + correct + '/' + scenarios.length + '</div>';
        h += '<button class="mg-btn" id="sim-done" style="pointer-events:auto;padding:10px 28px;margin-top:12px">' + (_ui.continue || 'Continuar →') + '</button>';
        h += '</div>';
      }
      h += '</div>';
      ct.innerHTML = h;
  ...
  ct.innerHTML = '<div style="font-family:inherit;padding:12px 16px;max-width:720px;margin:0 auto;color:' + txt + '">' +
    ...
    '<div id="fb-complete" style="display:none;text-align:center;margin-top:12px;padding:14px;background:rgba(144,224,160,0.06);border:1px solid ' + suc + ';border-radius:10px">' +
      '<div style="color:' + suc + ';font-size:16px;margin-bottom:8px">✓ ' + (_ui.complete || '¡Exploración completada!') + '</div>' +
      '<div style="font-size:12px;color:' + dim + ';margin-bottom:10px">' + subtitle + '</div>' +
      '<button class="mg-btn" id="fb-done" style="pointer-events:auto;padding:10px 28px">' + (_ui.continue || 'Continuar →') + '</button>' +
    '</div>' +
  '</div>';
  ...
  var zones = [
    {min:0, max:20, msg:(_ui.veryLow||'Rango muy bajo: efecto mínimo')},
    {min:20, max:40, msg:(_ui.low||'Rango bajo: el efecto comienza a aparecer')},
    {min:40, max:60, msg:(_ui.medium||'Rango medio: estado equilibrado')},
    {min:60, max:80, msg:(_ui.high||'Rango alto: se observa un efecto fuerte')},
    {min:80, max:100, msg:(_ui.veryHigh||'Rango muy alto: impacto máximo')}
  ];
  var visitedZones = {};
