(async function () {
  ...
  function refresh() {
    ...
    $('interaction').textContent = s.dialogue ? 'Luna está conversando contigo. El mundo sigue aquí.' : nearby ? `${nearby.label} · E para ${nearby.type === 'npc' ? 'conversar' : nearby.type === 'router' ? 'cambiar la ruta' : nearby.type === 'exit' ? 'continuar' : nearby.type === 'switch' ? 'activar' : 'usar la consola'}.` : 'Explora el distrito. Acércate a un objeto para interactuar.';
  ...
  function draw() {
    ...
    for (const p of engine.puzzles.values()) {
      ...
      for (const edge of p.mechanics.edges) {
        ...
        const active = s.routes[edge.source] === edge.target;
        ...
        if (active) {
          const phase = reduced.matches ? .5 : time / 1.6 % 1;
          rect(ax + (bx - ax) * phase - 3, ay + (by - ay) * phase - 3, 6, 6, s.solved ? '#e8ffd1' : '#e0bf7e');
    ...
    for (const e of actors) {
      ...
      if (e.type === 'goal') { rect(x + 2, y + 2, 44, 44, '#728c4960'); ctx.strokeStyle = '#b9ca7b'; ctx.lineWidth = 2; ctx.strokeRect(x + 4, y + 4, 40, 40); label(e.label, x + 24, y + 46, '#cddbad', 115); }
        ...
        const active = s?.solved || (e.type === 'switch' ? s?.sequence.includes(e.controlId) : !!s?.routes[e.controlId]);
        rect(x + 8, y + 8, 32, 33, '#0a1d25'); rect(x + 10, y + 4, 28, 28, '#4a686c'); rect(x + 14, y + 8, 20, 14, active ? '#86d9ab' : '#30494f');
        rect(x + 14, y + 26, 7, 3, s?.effect && !s.effect.ok ? '#ee977a' : '#d7bd7c');
        const text = e.type === 'router' ? `${e.controlId}${s?.routes[e.controlId] ? ' → ' + s.routes[e.controlId] : ''}` : e.type === 'console' ? 'ENVIAR / CONTROL' : e.label;
        label(text, x + 24, y - 6, active ? '#b2edc2' : '#b4ccd0', 120);
        if (e.type === 'router') { const n = engine.puzzles.get(e.puzzleId).mechanics.nodes.find(n => n.id === e.controlId); label(`carga ${s?.effect?.loads?.[e.controlId] || 0}/${n.capacity}`, x + 24, y + 49, '#d0bd91', 80); }
      }
    }
  ...
  try {
    ...
    if (config.debug === true) {
      ...
      window.__MINDCRAFTED_TEST__ = {getState: () => engine.snapshot(), getBKT: () => tracker.summary(), getCampaign: () => session?.snapshot()};
    }
