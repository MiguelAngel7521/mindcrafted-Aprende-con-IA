...
def assemble_course_platform(manifest: dict, output_dir: str, locale: str = DEFAULT_LOCALE) -> str:
    ...
    Args:
        ...
        locale: idioma de los textos de la interfaz (predeterminado: es).
...
def _build_games_cards(games: list[dict], output_dir: str, course_id: str = "", locale: str = "es") -> str:
...
<header class="header">
  <div class="header-inner">
    <a class="brand" href="/courses/" style="text-decoration:none;color:inherit">🎮 MindCrafted</a>
...
async function _checkCourseAccess() {
  ...
  if (result.reason === 'credit_required') {
    if (gateTitle) gateTitle.textContent = 'Desbloquear este curso';
    if (gateMsg) gateMsg.textContent = 'Este curso fue publicado por su creador. Usa créditos para desbloquear todo el contenido.';
    if (gateBtn) {
      gateBtn.textContent = 'Usar ' + (result.creditRequired || 0) + ' créditos para desbloquear';
      gateBtn.onclick = async function() {
        var r = await auth.payCreditsForCourse(COURSE_ID);
        if (r.success) { location.reload(); }
        else { authUI.showToast(r.error || 'No se pudo completar el pago'); }
      };
    }
  } else {
    if (gateTitle) gateTitle.textContent = 'Inicia sesión para acceder';
    if (gateMsg) gateMsg.textContent = 'Inicia sesión o crea una cuenta para acceder al curso. También puedes usar un enlace compartido para probar el primer juego.';
    if (gateBtn) {
      gateBtn.textContent = 'Iniciar sesión / Registrarse';
